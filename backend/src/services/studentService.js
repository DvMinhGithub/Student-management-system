...
module.exports = {
    ...
    addStudent: async ({ data }) => {
        try {
            ...
            if (existingStudent) return { code: 400, message: 'Sinh viên đã tồn tại' };
            else {
    ...
    updateStudent: async (studentId, body) => {
        try {
            const student = await Student.findByIdAndUpdate(studentId, { $set: body }, { new: true });
            return { code: 201, message: 'Cập nhật sinh viên thành công.', data: student };
        } catch (error) {
            console.error(error);
    ...
    cancelRegisterCourse: async ({ studentId, courseId }) => {
        try {
            const result = await Promise.all([
                Student.findOneAndUpdate({ _id: studentId }, { $pull: { courses: courseId } }),
                Course.findOneAndUpdate({ _id: courseId }, { $pull: { students: studentId } }),
            ]);
    ...
    uploadExcel: async ({ file }) => {
        try {
            ...
            for (const row of data) {
                const hashedPassword = await bcrypt.hash(row['Số điện thoại'].toString(), 10);
