...
module.exports = {
    ...
    updatedTeacher: async ({ teacherId, data }) => {
        try {
            ...
            const teacherUpdated = await Teacher.findByIdAndUpdate(teacherId, data, { new: true }).populate('courses');

            await Promise.all([
                Course.updateMany({ _id: { $in: coursesToRemove } }, { $pull: { teachers: teacherId } }),
                Course.updateMany({ _id: { $in: coursesToAdd } }, { $push: { teachers: teacherId } }),
            ]);
    ...
    uploadExcel: async ({ file }) => {
        try {
            ...
            for (const row of data) {
                const courseNames = row['Môn học'].includes(',') ? row['Môn học'].split(', ') : [row['Môn học']];
                const courseIds = await Course.find({ name: { $in: courseNames } }).select('_id');
